...
object LogisticRegressionWithSGDBinary {

  def main(args: Array[String]): Unit = {
    SetLogger
    val sc = new SparkContext(new SparkConf().setAppName("logistic.regression.binary").setMaster("local[4]"))
  }
  ...
  def trainEvaluate(trainData: RDD[LabeledPoint], validationData: RDD[LabeledPoint]): LogisticRegressionModel = {
    ...
    val (model, time) = trainModel(trainData, 500, 0.01, 0.1)
  ...
  def predictData(testData: RDD[LabeledPoint], model: LogisticRegressionModel, categoriesMap: Map[String, Int]) = {
